/* =========================================================
   PHARMA FIT — dicionário português do Brasil → espanhol do Paraguai

   Cada linha é uma frase da tela e a mesma frase em espanhol. O
   motor (idioma.js) troca só o que está aqui.

   O QUE NÃO ESTÁ AQUI, DE PROPÓSITO

   · nome, marca e dose de produto — "Tirzec Pen 15 mg" é o nome da
     caixa que o cliente vai receber. Traduzir viraria "Bolígrafo
     Tirzec" e ele pediria a coisa errada;
   · "PHARMA FIT" — marca não se traduz;
   · número de telefone e endereço.

   O REGISTRO É "USTED"

   O comércio paraguaio trata o cliente de "usted". O português do
   site é informal ("você"), mas traduzir informal por informal
   soaria malcriado numa loja de lá. Quem quiser mudar, muda aqui.

   PARA ACRESCENTAR UMA FRASE: acrescente uma linha. Não é preciso
   mexer em nenhuma das 16 páginas.

   A conferência  reclama se aparecer texto na
   tela que não esteja neste arquivo.
   ========================================================= */
window.PHARMAFIT_ES ={
  "(92) 8590-4669 · resposta em horário comercial": "(92) 8590-4669 · respuesta en horario comercial",
  "(xx) xxxxx-xxxx": "(xx) xxxxx-xxxx",
  ", não nos nossos servidores. Você pode apagar a qualquer momento nos botões dessas telas.": ", no en nuestros servidores. Puede borrarlos en cualquier momento con los botones de esas pantallas.",
  ". O andamento de cada um a gente confirma pelo WhatsApp.": ". El avance de cada uno lo confirmamos por WhatsApp.",
  ". É o que a nossa equipe usa para confirmar a venda com você.": ". Es lo que nuestro equipo usa para confirmar la venta con usted.",
  "0 itens": "0 ítems",
  "3 perguntas e indicamos seu protocolo": "3 preguntas y le indicamos su protocolo",
  "A equipe confere o pedido e combina o pagamento e a entrega com você.": "El equipo revisa el pedido y acuerda contigo el pago y la entrega.",
  "A equipe confirma com você o estoque, o valor final e a entrega antes de qualquer pagamento. Nada é cobrado por aqui.": "El equipo confirma con usted el stock, el valor final y la entrega antes de cualquier pago. Aquí no se cobra nada.",
  "A equipe precisa saber para quem responder e para onde entregar. Nada é cobrado por aqui.": "El equipo necesita saber a quién responder y dónde entregar. Aquí no se cobra nada.",
  "A equipe te ajuda a achar o que procura": "El equipo le ayuda a encontrar lo que busca",
  "A forma de entrega e o pagamento são combinados no WhatsApp, junto com a confirmação do pedido.": "La forma de entrega y el pago se acuerdan por WhatsApp, junto con la confirmación del pedido.",
  "A mesma senha": "La misma contraseña",
  "A página principal do site": "La página principal del sitio",
  "Abrindo o WhatsApp…": "Abriendo WhatsApp…",
  "Abrir menu": "Abrir menú",
  "Academia": "Gimnasio",
  "Acompanhamento": "Seguimiento",
  "Acompanhamento médico": "Seguimiento médico",
  "Acompanhe suas aplicações": "Siga sus aplicaciones",
  "Adicionar": "Agregar",
  "Adicionar ao carrinho": "Agregar al carrito",
  "Apagar histórico deste aparelho": "Borrar el historial de este dispositivo",
  "Apagar meus dados": "Borrar mis datos",
  "Apliquei hoje": "Apliqué hoy",
  "Apoio de vendas": "Apoyo de ventas",
  /* Os nomes das faixas de quantidade. "Varejo/Atacado" é o par que o
     comércio paraguaio escreve "Minorista/Mayorista". */
  "Atacado": "Mayorista",
  "Atacado +": "Mayorista +",
  "Atalhos": "Accesos rápidos",
  "Atendimento": "Atención",
  "Atendimento direto": "Atención directa",
  "Atendimento em Manaus · AM 🇧🇷": "Atención en Manaos · AM 🇧🇷",
  "Atendimento personalizado": "Atención personalizada",
  "Atendimento personalizado no Paraguai": "Atención personalizada en Paraguay",
  "Atendimento — Pharma Fit": "Atención — Pharma Fit",
  "Avise-me quando chegar": "Avíseme cuando llegue",
  "Bem-estar": "Bienestar",
  "Blend de peptídeos Klow, frasco de 70 mg.": "Mezcla de péptidos Klow, frasco de 70 mg.",
  "Buscar por produto": "Buscar por producto",
  "Buscar produtos": "Buscar productos",
  "Como comprar": "Cómo comprar",
  "CPF, CNPJ ou registro profissional": "CI, RUC o registro profesional",
  "Cadastro de parceiros": "Registro de socios",
  "Cadastro de parceiros e orçamentos": "Registro de socios y presupuestos",
  "Caneta aplicadora de tirzepatida 15 mg, pronta para uso.": "Lapicera aplicadora de tirzepatida 15 mg, lista para usar.",
  "Carregando o produto…": "Cargando el producto…",
  "Categorias de produtos": "Categorías de productos",
  "Categorias": "Categorías",
  "Cidade": "Ciudad",
  "Ciência. Segurança. Resultado.": "Ciencia. Seguridad. Resultado.",
  "Clínica": "Clínica",
  "Clínica, profissional ou parceiro": "Clínica, profesional o socio",
  "Com acompanhamento médico especializado": "Con seguimiento médico especializado",
  "Comprar": "Comprar",
  "Como funciona": "Cómo funciona",
  "Como pago?": "¿Cómo pago?",
  "Como podemos te chamar": "Cómo podemos llamarle",
  "Compartilhar": "Compartir",
  "Comprar em": "Comprar en",
  "Comprar em quantidade": "Comprar por cantidad",
  "Comprar em quantidade — Pharma Fit": "Comprar por cantidad — Pharma Fit",
  "Comprar para revender": "Comprar para revender",
  "Confirmamos tudo na conversa": "Confirmamos todo en la conversación",
  "Conta": "Cuenta",
  "Conte um pouco do seu trabalho": "Cuéntenos un poco de su trabajo",
  "Continuar": "Continuar",
  "Continuar escolhendo": "Seguir eligiendo",
  "Continuar no WhatsApp": "Seguir por WhatsApp",
  "Controle": "Control",
  "Controle de peso e apetite": "Control de peso y apetito",
  "Controle do apetite": "Control del apetito",
  "Criar conta": "Crear cuenta",
  "Criar conta — Pharma Fit": "Crear cuenta — Pharma Fit",
  "Criar sua conta": "Cree su cuenta",
  "Da mesma linha": "De la misma línea",
  "Deixe seus dados e continue no WhatsApp. Confirmamos tudo por lá antes de separar o pedido.": "Deje sus datos y siga por WhatsApp. Confirmamos todo allí antes de preparar el pedido.",
  "Descobrir meu protocolo": "Descubrir mi protocolo",
  "Descobrir meu protocolo — Pharma Fit": "Descubrir mi protocolo — Pharma Fit",
  "Diferenciais": "Diferenciales",
  "Dose, valor, forma de pagamento e endereço de entrega.": "Dosis, valor, forma de pago y dirección de entrega.",
  "E-mail": "Correo electrónico",
  "Ele pode ter saído do catálogo.": "Puede haber salido del catálogo.",
  "Emagrecer com acompanhamento": "Adelgazar con seguimiento",
  "Emagrecimento": "Adelgazamiento",
  "Empresa ou clínica": "Empresa o clínica",
  "Endereço copiado": "Dirección copiada",
  "Endereço de entrega": "Dirección de entrega",
  "Entrar": "Ingresar",
  "Entrar na conta": "Ingresar a la cuenta",
  "Entrar na fila": "Entrar en la fila",
  "Entrar na minha conta": "Ingresar a mi cuenta",
  "Entrar na sua conta": "Ingrese a su cuenta",
  "Entrar — Pharma Fit": "Ingresar — Pharma Fit",
  "Entrega": "Entrega",
  "Entrega e pagamento": "Entrega y pago",
  "Entrega segura": "Entrega segura",
  "Enviar cadastro": "Enviar registro",
  "Enviar pedido": "Enviar pedido",
  "Escolha a quantidade e toque em Adicionar. No carrinho você preenche nome, WhatsApp e endereço e finaliza o pedido.": "Elige la cantidad y toca Agregar. En el carrito completas nombre, WhatsApp y dirección y finalizas el pedido.",
  "Escolha o que mais combina com o seu momento.": "Elija lo que más combina con su momento.",
  "Escolha os produtos e eles aparecem aqui.": "Elija los productos y aparecen aquí.",
  "Escolha pelo menos um interesse — é o que a equipe usa para saber por onde começar a conversa.": "Elija al menos un interés — es lo que el equipo usa para saber por dónde empezar la conversación.",
  "Escolha pelo menos um interesse.": "Elija al menos un interés.",
  "Esconder a senha": "Ocultar la contraseña",
  "Esqueci minha senha": "Olvidé mi contraseña",
  "Estes são os pedidos feitos": "Estos son los pedidos hechos",
  "Estou em tratamento": "Estoy en tratamiento",
  "Ex.: 20": "Ej.: 20",
  "Falar agora no WhatsApp": "Hablar ahora por WhatsApp",
  "Falar com a equipe": "Hablar con el equipo",
  "Falar no WhatsApp": "Hablar por WhatsApp",
  "Favoritar": "Favorito",
  "Favoritos, histórico de pedidos e os dados que você salva em \"Minha conta\" ficam guardados": "Favoritos, historial de pedidos y los datos que guarda en \"Mi cuenta\" quedan almacenados",
  "Fechar": "Cerrar",
  "Fechar menu": "Cerrar menú",
  "Fechar pedido no WhatsApp": "Cerrar pedido por WhatsApp",
  /* a versão curta do mesmo botão, que aparece no celular estreito */
  "Fechar pedido": "Cerrar pedido",
  "Forma de pagamento, entrega, algo fora da lista…": "Forma de pago, entrega, algo fuera de la lista…",
  "Frasco de tirzepatida Pharma Fit ao lado de uma seringa": "Frasco de tirzepatida Pharma Fit al lado de una jeringa",
  "Garantias": "Garantías",
  "Gestão (equipe)": "Gestión (equipo)",
  "Glow GHK-Cu Alluvi": "Glow GHK-Cu Alluvi",
  "Guardamos só o seu nome, e-mail e WhatsApp — o necessário para a equipe confirmar o pedido.": "Guardamos solo su nombre, correo y WhatsApp — lo necesario para que el equipo confirme el pedido.",
  "Idioma do site": "Idioma del sitio",
  "Inicie seu protocolo": "Inicie su protocolo",
  "Início": "Inicio",
  "Ir para o produto": "Ir al producto",
  "Isso muda a dose que vamos indicar.": "Esto cambia la dosis que vamos a indicar.",
  "Já fiz antes": "Ya hice antes",
  "Já tem conta?": "¿Ya tiene cuenta?",
  "LINHA PREMIUM": "LÍNEA PREMIUM",
  "Limpar o carrinho": "Vaciar el carrito",

  /* PAGAR NO SITE, COM PIX. "PIX" não se traduz: é o nome do sistema
     brasileiro de pagamento, e quem vai pagar por ele reconhece o nome —
     traduzir viraria uma palavra que não existe em banco nenhum.
     As frases de erro estão aqui inteiras porque o tradutor procura frase
     completa no dicionário; cortar em pedaços faz o espanhol sair
     remendado. */
  "A equipe confirma o estoque e combina a entrega com você. Você pode pagar agora com PIX ou fechar o pedido no WhatsApp.":
    "El equipo confirma el stock y acuerda la entrega con usted. Puede pagar ahora con PIX o cerrar el pedido por WhatsApp.",
  "ou pague agora": "o pague ahora",
  "Pagar com PIX": "Pagar con PIX",
  "Gerando o código…": "Generando el código…",
  "O código aparece aqui na tela, na hora.": "El código aparece aquí en la pantalla, al instante.",
  "Copiar código": "Copiar código",
  "Copiado!": "¡Copiado!",
  "Copie o código selecionado": "Copie el código seleccionado",
  "Código QR do PIX desta compra": "Código QR del PIX de esta compra",
  "Esperando o pagamento…": "Esperando el pago…",
  "Este código vale por uma hora.": "Este código vale por una hora.",
  "O carrinho está vazio.": "El carrito está vacío.",
  "O código venceu. Toque em \"Pagar com PIX\" para gerar outro.":
    "El código venció. Toque en \"Pagar con PIX\" para generar otro.",
  "Não conseguimos abrir a compra agora. Feche o pedido no WhatsApp, logo abaixo — a equipe resolve com você.":
    "No pudimos abrir la compra ahora. Cierre el pedido por WhatsApp, abajo — el equipo lo resuelve con usted.",
  "Não deu para gerar o PIX agora. Feche no WhatsApp, logo abaixo.":
    "No se pudo generar el PIX ahora. Cierre por WhatsApp, abajo.",

  "Linha de peptídeos": "Línea de péptidos",
  "Lista de produtos": "Lista de productos",
  "MAIS PROCURADO": "MÁS BUSCADO",
  "MAIS VENDIDO": "MÁS VENDIDO",
  "Maior preço": "Mayor precio",
  "Mais prático": "Más práctico",
  "Mais vendidos": "Más vendidos",
  "Manaus · AM": "Manaos · AM",
  "Marque o dia em que aplicar: a gente calcula a próxima e você pode salvar o lembrete no calendário do celular. Fica só no seu aparelho.": "Marque el día en que aplique: calculamos la próxima y puede guardar el recordatorio en el calendario del celular. Queda solo en su dispositivo.",
  "Melhor custo por dose": "Mejor costo por dosis",
  "Menor preço": "Menor precio",
  "Menu": "Menú",
  "Mais": "Más",
  "Meu carrinho": "Mi carrito",
  /* o × que tira o produto do carrinho: é `aria-label`, só o leitor de
     tela lê — e ele lê em espanhol quando o site está em espanhol */
  "Tirar do carrinho": "Quitar del carrito",
  /* os dois ícones novos da barra de cima (18/09/2026). O conferidor
     de idioma lê `aria-label` também, e bem: é o que o leitor de tela
     anuncia, e um site em espanhol com o botão falando português é
     igualmente meio traduzido. */
  "Minha conta": "Mi cuenta",
  "Meu carrinho — Pharma Fit": "Mi carrito — Pharma Fit",
  "Meu protocolo": "Mi protocolo",
  "Meus dados": "Mis datos",
  "Meus favoritos": "Mis favoritos",
  "Meus favoritos — Pharma Fit": "Mis favoritos — Pharma Fit",
  "Meus pedidos": "Mis pedidos",
  "Meus pedidos — Pharma Fit": "Mis pedidos — Pharma Fit",
  "Minha conta — Pharma Fit": "Mi cuenta — Pharma Fit",
  "Monte o pedido escolhendo as quantidades. A equipe responde no WhatsApp com o valor fechado nas suas condições de parceiro.": "Arme el pedido eligiendo las cantidades. El equipo responde por WhatsApp con el valor cerrado en sus condiciones de socio.",
  "Mostrar a senha": "Mostrar la contraseña",
  "Nada é cobrado agora. A equipe confirma valores e prazo com você.": "Aquí no se cobra nada. El equipo confirma valores y plazo con usted.",
  "Nada é cobrado aqui no site.": "Nada se cobra aquí en el sitio.",
  "Navegação principal": "Navegación principal",
  "Nenhum pedido por aqui ainda": "Todavía no hay ningún pedido",
  "Neste mês": "Este mes",
  "Nome": "Nombre",
  "Nome completo": "Nombre completo",
  "Nos favoritos": "En favoritos",
  "Nos próximos 15 dias": "En los próximos 15 días",
  "Nossa seleção": "Nuestra selección",
  "Nosso time analisa e entra em contato": "Nuestro equipo analiza y le contacta",
  "Nossos protocolos": "Nuestros protocolos",
  "Não achamos este produto": "No encontramos este producto",
  "Não sabe por onde começar?": "¿No sabe por dónde empezar?",
  "Não sei o que escolher": "No sé qué elegir",
  "Não sei por onde começar": "No sé por dónde empezar",
  "Não tem conta?": "¿No tiene cuenta?",
  "Não vendemos, alugamos nem compartilhamos seus dados com terceiros para publicidade. Não usamos rastreadores de anúncios no site.": "No vendemos, alquilamos ni compartimos sus datos con terceros para publicidad. No usamos rastreadores de anuncios en el sitio.",
  "O endereço que você abriu não existe (ou saiu do ar). Vamos te levar de volta.": "La dirección que abrió no existe (o salió del aire). Le llevamos de vuelta.",
  "O pedido chega no WhatsApp da nossa equipe, que confere tudo e combina com você o pagamento e a entrega.": "El pedido llega al WhatsApp de nuestro equipo, que revisa todo y acuerda contigo el pago y la entrega.",
  "O prazo depende do seu endereço. Combinamos o prazo exato na conversa, antes de fechar o pedido.": "El plazo depende de su dirección. Acordamos el plazo exacto en la conversación, antes de cerrar el pedido.",
  "O produto que quero está sem estoque": "El producto que quiero está sin stock",
  "O quanto antes": "Cuanto antes",
  "O que fica só no seu aparelho": "Lo que queda solo en su dispositivo",
  "O que guardamos": "Lo que guardamos",
  "O que guardamos e o que fica só com você": "Lo que guardamos y lo que queda solo con usted",
  "O que não fazemos": "Lo que no hacemos",
  "O que você escolheu até agora.": "Lo que eligió hasta ahora.",
  "O que você pediu pelo site, guardado neste aparelho.": "Lo que pidió por el sitio, guardado en este dispositivo.",
  "O que você procura agora?": "¿Qué busca ahora?",
  "Objetivo principal": "Objetivo principal",
  "Observações": "Observaciones",
  "Oferecemos protocolos exclusivos com medicamentos originais, acompanhamento personalizado e suporte completo antes, durante e após o tratamento.": "Ofrecemos protocolos exclusivos con medicamentos originales, seguimiento personalizado y soporte completo antes, durante y después del tratamiento.",
  "Onde você está": "Dónde está",
  "Opcional": "Opcional",
  "Ordenar por": "Ordenar por",
  "Orçamento de atacado": "Presupuesto por mayor",
  "Orçamento de atacado para pedidos maiores": "Presupuesto por mayor para pedidos grandes",
  "Os preços abaixo são os de tabela do site — servem de referência para você montar o pedido.": "Los precios de abajo son los de lista del sitio — sirven de referencia para armar el pedido.",
  "Os produtos mais escolhidos por quem busca resultados reais e consistentes.": "Los productos más elegidos por quienes buscan resultados reales y consistentes.",
  "Os produtos são de uso sob orientação profissional. Fale com a equipe no WhatsApp que explicamos como funciona no seu caso.": "Los productos son de uso bajo orientación profesional. Hable con el equipo por WhatsApp y le explicamos cómo funciona en su caso.",
  "Os produtos vendidos são de uso sob orientação profissional. As informações do site não substituem consulta, diagnóstico ou prescrição. Resultados variam de pessoa para pessoa.": "Los productos vendidos son de uso bajo orientación profesional. La información del sitio no sustituye consulta, diagnóstico ni prescripción. Los resultados varían de persona a persona.",
  "Outro parceiro": "Otro socio",
  "Outros produtos, forma de pagamento, entrega…": "Otros productos, forma de pago, entrega…",
  "Pedido enviado para a equipe. A confirmação do estoque, do valor final e da entrega vem na conversa do WhatsApp.": "Pedido enviado al equipo. La confirmación del stock, del valor final y de la entrega llega en la conversación de WhatsApp.",
  "Pedido confirmado no WhatsApp": "Pedido confirmado por WhatsApp",
  "PROMOÇÃO": "PROMOCIÓN",
  "Para acompanhar seus pedidos": "Para seguir sus pedidos",
  "Para acompanhar seus pedidos e suas compras.": "Para seguir sus pedidos y sus compras.",
  "Para acompanhar seus pedidos em qualquer aparelho.": "Para seguir sus pedidos en cualquier dispositivo.",
  "Para onde vai o pedido": "Adónde va el pedido",
  "Para quando precisa?": "¿Para cuándo lo necesita?",
  "Para quem já fez tratamento antes": "Para quien ya hizo tratamiento antes",
  "Paraguai": "Paraguay",
  /* as três frases do cartão da gestão, na Conta. Ele só aparece para
     quem é da equipe, mas o site tem dois idiomas e a regra não abre
     exceção: meio traduzido é pior que sem tradução. */
  "Acesso da equipe": "Acceso del equipo",
  "Abrir o painel": "Abrir el panel",
  "Painel de gestão": "Panel de gestión",
  "Pedidos, produtos, clientes e relatórios": "Pedidos, productos, clientes e informes",
  "Pedidos": "Pedidos",
  "Pedidos maiores têm preço e prazo próprios. Diga o que precisa que a equipe monta o orçamento.": "Los pedidos grandes tienen precio y plazo propios. Diga lo que necesita y el equipo arma el presupuesto.",
  "Pedir no WhatsApp": "Pedir por WhatsApp",
  "Pedir orçamento": "Pedir presupuesto",
  "Pedir orçamento de atacado": "Pedir presupuesto por mayor",
  "Pele, cabelo e recuperação": "Piel, cabello y recuperación",
  "Pelo menos 8 caracteres": "Al menos 8 caracteres",
  "Senha nova": "Contraseña nueva",
  "Repita a senha nova": "Repita la contraseña nueva",
  "Salvar senha nova": "Guardar contraseña nueva",
  "Criar senha nova": "Crear contraseña nueva",
  "A senha precisa de pelo menos 8 caracteres.": "La contraseña necesita al menos 8 caracteres.",
  "As duas senhas não são iguais.": "Las dos contraseñas no son iguales.",
  "O link expirou ou já foi usado. Peça outro em \"Esqueci minha senha\".": "El enlace expiró o ya fue usado. Pida otro en \"Olvidé mi contraseña\".",
  "Senha nova salva. Abrindo a sua conta…": "Contraseña nueva guardada. Abriendo su cuenta…",
  "Pelo site ou com a nossa ajuda no WhatsApp.": "Por el sitio o con nuestra ayuda por WhatsApp.",
  "Peptídeo de cobre GHK-Cu, frasco de 100 mg.": "Péptido de cobre GHK-Cu, frasco de 100 mg.",
  "Peptídeo de cobre GHK-Cu, linha Alluvi.": "Péptido de cobre GHK-Cu, línea Alluvi.",
  "Peptídeos": "Péptidos",
  "Performance": "Rendimiento",
  "Performance e energia": "Rendimiento y energía",
  "Perguntas frequentes": "Preguntas frecuentes",
  "Pharma Fit · Saúde, performance e bem-estar — atendimento em Manaus · AM 🇧🇷": "Pharma Fit · Salud, rendimiento y bienestar — atención en Manaos · AM 🇧🇷",
  "Pharma Fit — Saúde, performance e bem-estar": "Pharma Fit — Salud, rendimiento y bienestar",
  "Pharma Fit — página inicial": "Pharma Fit — página de inicio",
  "Pix, cartão de crédito ou débito, dinheiro e transferência. Parcelamos em até 3x sem juros.": "Pix, tarjeta de crédito o débito, efectivo y transferencia. Financiamos hasta en 3 cuotas sin interés.",
  "Preciso de receita?": "¿Necesito receta?",
  "Preencha": "Complete",
  "Preencha seus dados e fale com nosso time especializado para iniciar seu tratamento com segurança, ciência e acompanhamento.": "Complete sus datos y hable con nuestro equipo especializado para iniciar su tratamiento con seguridad, ciencia y seguimiento.",
  "Preço de parceiro": "Precio de socio",
  "Privacidade": "Privacidad",
  "Produto": "Producto",
  "Produto de interesse": "Producto de interés",
  "Produto retirado do protocolo.": "Producto quitado del protocolo.",
  "Produto — Pharma Fit": "Producto — Pharma Fit",
  /* A ABA DA BARRA DE BAIXO VOLTOU A SER "Produtos".
     Ela tinha virado "Protocolos" por causa de um desenho que o Brian
     mandou; em 18/09/2026 ele pediu de volta: "mude o nome de
     protocolos para produtos". A entrada de "Protocolos" fica aqui de
     propósito — a palavra continua no site no sentido de TRATAMENTO
     ("3 perguntas e indicamos seu protocolo", "Inicie seu protocolo"),
     e é esse sentido que ela traduz agora, não o da aba. */
  "Protocolos": "Protocolos",
  "Produtos": "Productos",
  "Produtos de uso sob orientação profissional.": "Productos de uso bajo orientación profesional.",
  "Produtos de uso sob orientação profissional. Resultados variam de pessoa para pessoa.": "Productos de uso bajo orientación profesional. Los resultados varían de persona a persona.",
  "Resultados variam de pessoa para pessoa.": "Los resultados varían de persona a persona.",
  "Produtos — Pharma Fit": "Productos — Pharma Fit",
  "Profissional da saúde": "Profesional de la salud",
  "Promoções primeiro": "Promociones primero",
  "Protocolo mais avançado": "Protocolo más avanzado",
  "Página não": "Página no",
  "Página não encontrada — Pharma Fit": "Página no encontrada — Pharma Fit",
  "Pôr um a mais": "Agregar uno más",
  "Quando você faz um pedido pelo site, guardamos apenas o necessário para atender:": "Cuando hace un pedido por el sitio, guardamos solo lo necesario para atenderlo:",
  "Quando você fizer um pedido pelo site, ele aparece nesta lista.": "Cuando haga un pedido por el sitio, aparece en esta lista.",
  "Quantidade": "Cantidad",
  "Quanto maior a quantidade, melhor o preço por unidade: as faixas estão logo acima, e o valor do seu pedido aparece antes de finalizar.": "Cuanta más cantidad, mejor el precio por unidad: los tramos están justo arriba, y el valor de tu pedido aparece antes de finalizar.",
  "Quanto maior o volume, melhor a condição. Responderemos no seu WhatsApp.": "Cuanto mayor el volumen, mejor la condición. Le respondemos a su WhatsApp.",
  "Quanto tempo demora a entrega?": "¿Cuánto tarda la entrega?",
  "Quantos atendimentos por mês, quais produtos usa hoje…": "Cuántas atenciones por mes, qué productos usa hoy…",
  "Quero a recomendação de vocês": "Quiero su recomendación",
  "Quero continuar ou trocar": "Quiero seguir o cambiar",
  "Quero falar com a equipe": "Quiero hablar con el equipo",
  "Quero meu atendimento": "Quiero mi atención",
  "R$ 0,00 na tabela": "R$ 0,00 en la lista",
  "Receba orientação": "Reciba orientación",
  "Redução de peso": "Reducción de peso",
  "Remover dos favoritos": "Quitar de favoritos",
  "Repetir a senha": "Repetir la contraseña",
  "Responda 3 perguntas e veja a indicação": "Responda 3 preguntas y vea la indicación",
  "Resumo do pedido": "Resumen del pedido",
  "Retatrutida": "Retatrutida",
  "Retatrutida 120 mg, linha ZPHC.": "Retatrutida 120 mg, línea ZPHC.",
  "Revendedor": "Revendedor",
  "Revendedores, clínicas e profissionais": "Revendedores, clínicas y profesionales",
  "Revendedores, clínicas, profissionais e parceiros comerciais têm condições próprias e atendimento direto.": "Revendedores, clínicas, profesionales y socios comerciales tienen condiciones propias y atención directa.",
  "Rua, número, bairro e complemento": "Calle, número, barrio y complemento",
  "SAÚDE · PERFORMANCE · BEM-ESTAR": "SALUD · RENDIMIENTO · BIENESTAR",
  "SAÚDE • PERFORMANCE • BEM-ESTAR": "SALUD • RENDIMIENTO • BIENESTAR",
  "Se preferir, toque em Pedir no WhatsApp e a conversa já abre com este produto e a quantidade dentro.": "Si prefieres, toca Pedir por WhatsApp y la conversación ya se abre con este producto y la cantidad dentro.",
  "SELEÇÃO PREMIUM": "SELECCIÓN PREMIUM",
  "Sair da conta": "Salir de la cuenta",
  "Salvar no aparelho": "Guardar en el dispositivo",
  "Salvar nos favoritos": "Guardar en favoritos",
  "Salve seus dados para não digitar tudo de novo no próximo pedido. Fica guardado": "Guarde sus datos para no escribir todo de nuevo en el próximo pedido. Queda guardado",
  "Salvos neste aparelho, sem precisar criar conta.": "Guardados en este dispositivo, sin necesidad de crear cuenta.",
  "Saúde · Performance · Bem-estar": "Salud · Rendimiento · Bienestar",
  "Se não souber, escolha \"tanto faz\" que a equipe orienta.": "Si no sabe, elija \"da igual\" y el equipo le orienta.",
  "Se você preenche o cadastro de representante ou pede um orçamento de atacado, os dados informados ficam disponíveis para a equipe comercial responder seu contato.": "Si completa el registro de representante o pide un presupuesto por mayor, los datos informados quedan disponibles para que el equipo comercial responda su contacto.",
  "Seguimos disponíveis durante todo o protocolo.": "Seguimos disponibles durante todo el protocolo.",
  "Seja um": "Sea un",
  "Seja um representante": "Sea un representante",
  "Seja um representante — Pharma Fit": "Sea un representante — Pharma Fit",
  "Selecione seu objetivo": "Seleccione su objetivo",
  "Sem estoque": "Sin stock",
  "Senha": "Contraseña",
  "Separação e envio": "Preparación y envío",
  "Seu WhatsApp": "Su WhatsApp",
  "Seu atendimento": "Su atención",
  "Seu carrinho está vazio": "Su carrito está vacío",
  "Seu nome": "Su nombre",
  "Seu nome completo": "Su nombre completo",
  "Seus dados": "Sus datos",
  "Seus dados estão protegidos e não serão compartilhados.": "Sus datos están protegidos y no serán compartidos.",
  "Seus dados vão direto para a equipe comercial. Sem compromisso.": "Sus datos van directo al equipo comercial. Sin compromiso.",
  "Seus pedidos e seus dados, guardados na sua conta e disponíveis em qualquer aparelho. Sem conta, eles ficam só neste aparelho.": "Sus pedidos y sus datos, guardados en su cuenta y disponibles en cualquier dispositivo. Sin cuenta, quedan solo en este dispositivo.",
  "Seus dados — Pharma Fit": "Sus datos — Pharma Fit",
  "Seus direitos": "Sus derechos",
  "Seus pedidos, suas aplicações e seus dados — tudo neste aparelho, sem cadastro.": "Sus pedidos, sus aplicaciones y sus datos — todo en este dispositivo, sin registro.",
  "Sim. Ficamos em Manaus (AM) e enviamos para outras cidades — o frete e o prazo entram no orçamento.": "Sí. Estamos en Manaos (AM) y enviamos a otras ciudades — el flete y el plazo entran en el presupuesto.",
  "Solicitar atendimento": "Solicitar atención",
  "Soluções premium para transformar sua saúde, performance e bem-estar.": "Soluciones premium para transformar su salud, rendimiento y bienestar.",
  "Sou profissional ou clínica": "Soy profesional o clínica",
  "Sua cidade": "Su ciudad",
  "Sua escolha premium em Tirzepatida, Retatrutida e Peptídeos. Entrega grátis em Manaus": "Su elección premium en Tirzepatida, Retatrutida y Péptidos. Envío gratis en Manaos",
  "Sua senha": "Su contraseña",
  "Suas condições são combinadas no WhatsApp.": "Sus condiciones se acuerdan por WhatsApp.",
  "Suas informações com segurança": "Sus datos con seguridad",
  "Só cotando por enquanto": "Solo cotizando por ahora",
  "Tanto faz": "Da igual",
  "Tenho interesse em:": "Me interesa:",
  "Tentar de novo": "Intentar de nuevo",
  "Tirar dúvidas ou pedir de novo": "Resolver dudas o pedir de nuevo",
  "Tirar um": "Quitar uno",
  "Tire suas dúvidas com quem entende do assunto — respondemos no WhatsApp.": "Resuelva sus dudas con quien entiende del tema — respondemos por WhatsApp.",
  "Tirzepatida": "Tirzepatida",
  "Tirzepatida 15 mg, caixa com 4 ampolas.": "Tirzepatida 15 mg, caja con 4 ampollas.",
  "Tirzepatida, peptídeos e retatrutida": "Tirzepatida, péptidos y retatrutida",
  "Todos": "Todos",
  "Toque em \"Avise-me quando chegar\" na página do produto. Você entra na fila e avisamos no WhatsApp assim que repor.": "Toque en \"Avíseme cuando llegue\" en la página del producto. Entra en la fila y le avisamos por WhatsApp en cuanto repongamos.",
  "Toque no coração de qualquer produto para guardá-lo aqui.": "Toque el corazón de cualquier producto para guardarlo aquí.",
  "Total": "Total",
  "Transparência sobre o que guardamos e o que fica só com você.": "Transparencia sobre lo que guardamos y lo que queda solo con usted.",
  "Tratamentos com": "Tratamientos con",
  "Três perguntas rápidas. No fim, mostramos a indicação e você fala com a equipe.": "Tres preguntas rápidas. Al final, mostramos la indicación y usted habla con el equipo.",
  "Cupom": "Cupón",
  "Cupom de desconto": "Cupón de descuento",
  "Aplicar": "Aplicar",
  "Tem cupom? Digite o código": "¿Tiene cupón? Escriba el código",
  "Conferindo…": "Verificando…",
  "Digite o código do cupom.": "Escriba el código del cupón.",
  "Esse código não existe. Confira como ele foi escrito.": "Ese código no existe. Verifique cómo fue escrito.",
  "Esse cupom não existe ou já não vale mais.": "Ese cupón no existe o ya no es válido.",
  "Não consegui conferir o cupom agora. Tente de novo em instantes.": "No pude verificar el cupón ahora. Intente de nuevo en unos instantes.",
  "Falar com a Pharma Fit no WhatsApp": "Hablar con Pharma Fit por WhatsApp",
  "Atendimento personalizado em Manaus": "Atención personalizada en Manaos",
  "Gostaria de mais informações": "Quisiera más información",
  "Varejo": "Minorista",
  "Vai levar bastante quantidade?": "¿Va a llevar bastante cantidad?",
  "Valores e condições sob consulta. Produtos de uso sob orientação profissional.": "Valores y condiciones a consultar. Productos de uso bajo orientación profesional.",
  "Valores e condições sob consulta.": "Valores y condiciones a consultar.",
  "Vamos achar": "Vamos a encontrar",
  "Ver detalhes": "Ver detalles",
  "Ver meus pedidos": "Ver mis pedidos",
  "Ver o que fazemos com seus dados": "Ver qué hacemos con sus datos",
  "Ver os produtos": "Ver los productos",
  "Ver produtos": "Ver productos",
  "Ver seleção": "Ver selección",
  "Ver todos": "Ver todos",
  "Ver tudo": "Ver todo",
  /* o cabeçalho da fila da página inicial */
  "Em destaque": "Destacados",
  "Ver todos os produtos": "Ver todos los productos",
  "Você ainda não fez pedidos por aqui.": "Todavía no hizo pedidos por aquí.",
  "Você ainda não salvou nada": "Todavía no guardó nada",
  "Você entra na fila de": "Entra en la fila de",
  "Você escolhe o produto": "Usted elige el producto",
  "Você já fez algum protocolo?": "¿Ya hizo algún protocolo?",
  "Você pode pedir a qualquer momento para ver, corrigir ou apagar os dados que temos sobre você. É só chamar no WhatsApp": "Puede pedir en cualquier momento ver, corregir o borrar los datos que tenemos sobre usted. Basta escribir al WhatsApp",
  "Você recebe o código de acompanhamento assim que despachar.": "Recibe el código de seguimiento en cuanto despachamos.",
  "Você está na conta": "Usted está en la cuenta",
  "Você é": "Usted es",
  "Vocês atendem fora de Manaus?": "¿Atienden fuera de Manaos?",
  "Voltar": "Volver",
  "Voltar ao início": "Volver al inicio",
  "Vários produtos": "Varios productos",
  "WhatsApp": "WhatsApp",
  "WhatsApp (92) 8590-4669": "WhatsApp (92) 8590-4669",
  "acompanhamento": "seguimiento",
  "aplicações": "aplicaciones",
  "canal exclusivo no WhatsApp": "canal exclusivo por WhatsApp",
  "combinada no WhatsApp": "acordada por WhatsApp",
  "começa agora": "empieza ahora",
  "começa aqui": "empieza aquí",
  "de peso": "de peso",
  "e avisamos no WhatsApp assim que chegar. Sem cadastro e sem compromisso.": "y le avisamos por WhatsApp en cuanto llegue. Sin registro y sin compromiso.",
  "e suporte": "y soporte",
  "encontrada": "encontrada",
  "especializado": "especializado",
  /* AQUI MORAVA O TEXTO DE APOIO DO TOPO, a frase que terminava em
     "Atendimento no Paraguai 🇵🇾". Ela saiu da tela em 18/09/2026
     ("Tire essas escritas"), e a tradução saiu junto: entrada de
     dicionário sem frase na tela não traduz nada e ainda promete que
     alguma tela diz aquilo. No dia de mexer, alguém a leria como
     texto vivo e iria procurar onde ela aparece. */
  "favorito": "favorito",
  "favoritos": "favoritos",

  /* ---- A PÁGINA DE CONTA, REFEITA EM 17/09/2026 ----
     "Faça uma parte de conta bem feita, isso aqui tá horrível." Os
     blocos novos são escritos pelo JavaScript, e texto escrito na hora
     é o que mais escapa da tradução: nenhuma leitura de arquivo HTML
     acha. Quem achou estas quatro primeiras foi a medição que abre as
     16 páginas em espanhol e procura português na tela. */
  "Entre na sua conta": "Entre en su cuenta",
  "Seus pedidos e seus dados de entrega ficam guardados e aparecem em qualquer aparelho — no celular e no computador.": "Sus pedidos y sus datos de entrega quedan guardados y aparecen en cualquier dispositivo — en el celular y en la computadora.",
  "Criar minha conta": "Crear mi cuenta",
  "Só quero ver os produtos": "Solo quiero ver los productos",
  "Você ainda não fez nenhum pedido": "Todavía no hizo ningún pedido",
  "Quando fizer, ele aparece aqui com o andamento — e em qualquer aparelho onde você entrar.": "Cuando lo haga, aparece aquí con el seguimiento — y en cualquier dispositivo donde entre.",
  "Escolha um produto e a equipe fecha com você pelo WhatsApp.": "Elija un producto y el equipo cierra con usted por WhatsApp.",
  "Ver os produtos": "Ver los productos",

  /* E TRÊS QUE JÁ FALTAVAM ANTES DE EU MEXER.
     A lista de pedidos existe desde o começo e estas frases nunca
     entraram no dicionário. Elas não apareciam na medição porque ela
     abre a Conta SEM pedido nenhum — e sem pedido a lista não é
     desenhada. Buraco que só aparece para quem comprou. */
  "Pedir de novo": "Pedir de nuevo",
  "Ver todos os pedidos": "Ver todos los pedidos",
  "pedido": "pedido",
  "materiais e orientação": "materiales y orientación",
  "na tabela": "en la lista",
  "neste aparelho": "en este dispositivo",
  "no seu próprio navegador": "en su propio navegador",
  "nome, WhatsApp, endereço de entrega, produto e quantidade": "nombre, WhatsApp, dirección de entrega, producto y cantidad",
  "o seu protocolo": "su protocolo",
  "parceiro": "socio",
  "pedidos": "pedidos",
  "performance": "rendimiento",
  "premium": "premium",
  "quantidade": "cantidad",
  "representante": "representante",
  "saúde": "salud",
  "seu@email.com": "su@correo.com",
  "só neste aparelho": "solo en este dispositivo",
  "tabela própria por volume": "lista propia por volumen",
  "tirar": "quitar",
  "tirzepatida": "tirzepatida",
  "Área do": "Área del",
  "Área do parceiro — Pharma Fit": "Área del socio — Pharma Fit",
  "É a primeira vez": "Es la primera vez",
  "É por aqui que a equipe confirma o seu pedido.": "Por aquí el equipo confirma su pedido.",
  "Últimos pedidos": "Últimos pedidos",

  /* a lista de pedidos: da conta ou do aparelho, e o andamento
     (postas em 16/09/2026, quando a lista passou a vir da conta) */
  "Estes são os pedidos da": "Estos son los pedidos de",
  "sua conta": "su cuenta",
  ", e aparecem em qualquer aparelho. O andamento de cada um a gente confirma pelo WhatsApp.": ", y aparecen en cualquier dispositivo. El avance de cada uno lo confirmamos por WhatsApp.",
  ". Entre na sua conta para vê-los em qualquer lugar.": ". Ingrese a su cuenta para verlos en cualquier lugar.",
  "Esperando a confirmação da equipe": "Esperando la confirmación del equipo",
  "Confirmado pela equipe": "Confirmado por el equipo",
  "Cancelado": "Cancelado"
};


/* ---------------------------------------------------------------
   OS PADRÕES — frase com número ou nome dentro

   "ou em até 3x sem juros de R$ 366,33" nunca casaria por texto
   exato: o valor muda a cada produto. Aqui o que varia fica em $1,
   $2… e volta no lugar certo.

   `pt` é a expressão que reconhece a frase; `es` é como ela sai,
   com `$1` e `$2` no lugar de cada pedaço que varia.
   --------------------------------------------------------------- */
window.PHARMAFIT_ES_PADROES = [
  /* "ou até 3x sem juros de R$ 366,33". O "em" saiu em 18/09/2026:
     com ele a frase quebrava em duas linhas no cartão da vitrine.
     O padrão antigo fica logo abaixo porque a frase com "em" ainda
     pode estar viva num cartão guardado no cache do navegador de
     alguém — padrão que sobra não custa nada; frase sem tradução
     aparece em português na tela do cliente paraguaio. */
  { pt: /^ou (\d+)x sem juros de (.+)$/, es: "o $1 cuotas sin interés de $2" },
  /* A PARCELA DO CARTÃO VIROU DOIS PEDAÇOS (18/09/2026), para o "3x
     sem juros" poder ficar em destaque: o tradutor casa frase inteira
     por nó de texto, então cada pedaço precisa do seu padrão. */
  { pt: /^(\d+)x sem juros$/, es: "$1 cuotas sin interés" },
  /* PARCELA COM JUROS (24/09/2026): quando a equipe escolhe no painel
     um valor de parcela que soma mais que o preço, a frase sai sem o
     "sem juros" — e precisa do seu próprio padrão. */
  { pt: /^ou (\d+)x de (.+)$/, es: "o $1 cuotas de $2" },
  { pt: /^(\d+)x$/, es: "$1 cuotas" },
  /* O RÓTULO DA BOLINHA DO CARRINHO, em cada cartão da vitrine. Ele
     nasceu em 18/09/2026, quando o botão escrito virou símbolo: o nome
     do produto entra dentro da frase, então ela nunca estaria no
     dicionário — é padrão, não frase. Sem isto, o leitor de tela do
     cliente paraguaio anunciava onze botões em português. */
  { pt: /^Adicionar (.+) ao carrinho$/, es: "Agregar $1 al carrito" },
  { pt: /^de (R\$ .+)$/, es: "de $1" },
  { pt: /^ou até (\d+)x sem juros de (.+)$/, es: "o hasta $1 cuotas sin interés de $2" },
  { pt: /^ou em até (\d+)x sem juros de (.+)$/, es: "o en hasta $1 cuotas sin interés de $2" },
  { pt: /^Bom dia, (.+)$/, es: "Buenos días, $1" },
  { pt: /^Boa tarde, (.+)$/, es: "Buenas tardes, $1" },
  { pt: /^Boa noite, (.+)$/, es: "Buenas noches, $1" },
  { pt: /^Minha conta, (.+)$/, es: "Mi cuenta, $1" },
  { pt: /^Ir para o produto (\d+)$/, es: "Ir al producto $1" },
  { pt: /^Pergunta (\d+) de (\d+)$/, es: "Pregunta $1 de $2" },
  { pt: /^(\d+) itens?$/, es: "$1 ítems" },
  { pt: /^(\d+) unid\.$/, es: "$1 unid." },
  /* as faixas encurtaram para caber em três colunas (18/09/2026):
     "unid." virou "un.", que sai igual nas duas línguas */
  { pt: /^(\d+) un\.$/, es: "$1 un." },
  { pt: /^(\d+) a (\d+) un\.$/, es: "$1 a $2 un." },

  /* "7 produtos" / "1 produto" embaixo de cada ícone de categoria, na
     página inicial. O número vem do catálogo, então a frase inteira
     nunca estaria no dicionário — é padrão, não frase. */
  { pt: /^(\d+) produtos$/, es: "$1 productos" },
  { pt: /^1 produto$/, es: "1 producto" },

  /* AS FAIXAS DE ATACADO.
     Entraram no site em 16/09 e eu esqueci o espanhol: quem abria a
     página do Tirzec Pen em espanhol via a tabela de preços em
     português — "Varejo", "Atacado", "10 unid. ou mais". Nenhuma
     leitura de arquivo acharia, porque a tabela é escrita pelo
     JavaScript na hora; e o meu conferidor de tela deixou passar
     "10 unid. ou mais", porque a isenção dele dispensava tudo que
     começasse com número seguido de "unid".
     "1 a 4 unid." não entra aqui: sai igual nas duas línguas. */
  { pt: /^(\d+) unid\. ou mais$/, es: "$1 unid. o más" },
  { pt: /^(\d+) un\. ou mais$/, es: "$1 un. o más" },
  { pt: /^Levando (\d+) unidade a mais, cada uma sai por (.+)\.$/,
    es: "Llevando $1 unidad más, cada una sale por $2." },
  { pt: /^Levando (\d+) unidades a mais, cada uma sai por (.+)\.$/,
    es: "Llevando $1 unidades más, cada una sale por $2." },
  { pt: /^Tirar uma unidade de (.+)$/, es: "Quitar una unidad de $1" },
  { pt: /^Somar uma unidade de (.+)$/, es: "Sumar una unidad de $1" },
  { pt: /^Quantidade de (.+)$/, es: "Cantidad de $1" },
  { pt: /^Tirar (.+) dos favoritos$/, es: "Quitar $1 de favoritos" },
  { pt: /^Salvar (.+) nos favoritos$/, es: "Guardar $1 en favoritos" },
  { pt: /^(\d+) em estoque$/, es: "$1 en stock" },

  /* A PÁGINA DE CONTA: o que tem número, e-mail ou data dentro.
     O nome do mês já sai em espanhol — `cliente.js` formata a data com
     o idioma da página em vez de fixar pt-BR, senão "Cliente desde
     septiembre" viria "setembro" no meio do espanhol. */
  { pt: /^Cliente desde (.+)$/, es: "Cliente desde $1" },
  { pt: /^Ver todos os (\d+) pedidos$/, es: "Ver todos los $1 pedidos" },
  { pt: /^Enviado em (.+)$/, es: "Enviado el $1" }
];
